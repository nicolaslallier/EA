...
describe('centreOf and placeCentredAt', () => {
  it('reads a stored position as the top-left of the box', () => {
    expect(centreOf({ x: 200, y: 100 })).toEqual({ x: 200 + BOX_WIDTH / 2, y: 100 + BOX_HEIGHT / 2 })
  })

  it('places a dropped box centred on the pointer', () => {
    const topLeft = placeCentredAt({ x: 400, y: 300 })

    expect(centreOf(topLeft)).toEqual({ x: 400, y: 300 })
  })
...
describe('clipToBox', () => {
  const box = { x: 200, y: 100 } // centre (266, 123)
...
describe('edgeSegment', () => {
  it('runs from border to border between two boxes side by side', () => {
    expect(edgeSegment({ x: 0, y: 0 }, { x: 300, y: 0 })).toEqual({
  ...
  it('draws nothing when the boxes overlap', () => {
    expect(edgeSegment({ x: 0, y: 0 }, { x: 100, y: 20 })).toBeNull()
  })
})
...
describe('canvasExtent', () => {
  ...
  it('stays at the minimum while every box fits inside it', () => {
    expect(canvasExtent([{ x: 100, y: 100 }], 36)).toEqual(MIN_CANVAS)
  })

  it('grows to contain the farthest box plus the margin', () => {
    expect(canvasExtent([{ x: 50, y: 700 }, { x: 1000, y: 10 }], 36)).toEqual({
      width: 1000 + BOX_WIDTH + 36,
